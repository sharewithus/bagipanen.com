...
@section('title', 'Satu langkah lagi bantu mereka')
@push('after-style')
    ...
    <div class="header flex flex-col overflow-hidden h-[220px] relative">
        <nav class="pt-5 px-3 flex justify-between items-center relative z-20">
            ...
            <div class="flex flex-col items-center text-center">
                <p class="font-semibold text-sm">#AyoMakmurkanMereka</p>
            </div>
    ...
    <div class="flex flex-col z-30">
        <div id="content" class="w-full min-h-[calc(100vh-220px)] h-full bg-white rounded-t-[40px] flex flex-col gap-5 p-[30px_24px_30px]">
            {{-- <form method="POST" action="{{route('front.store', ['fundraising' => $fundraising->slug, 'totalAmountDonation' => $totalAmountDonation])}}" class="flex flex-col gap-5" enctype="multipart/form-data">
                @csrf --}}
            <form class="flex flex-col gap-5" id="form-donasi" onsubmit="return submitForm();">
                {{-- @csrf --}}
                <div class="flex flex-col gap-[10px]">
                    ...
                    <input type="hidden" id="fundraising_id" name="fundraising_id" value="{{$fundraising->id}}">
                    <input type="hidden" id="fundraising_slug" name="fundraising_slug" value="{{$fundraising->slug}}">
                </div>
                {{-- <hr class="border-dashed">
                <button type="button" id="pay-button" class="p-[14px_20px] bg-[#4541FF] rounded-full text-white w-full mx-auto font-semibold hover:shadow-[0_12px_20px_0_#76AE4380] transition-all duration-300 text-nowrap text-center">Donasi</button>
                <div class="flex flex-col gap-[10px]">

                    <p class="font-semibold text-xs text-center">Atau</p>
                    <p class="font-semibold text-sm">Kirim Donasi Kamu Kesini</p>
                        ...
                        <div id="accordion-faq-1" class="accordion-content open">
                            <div class="pt-3 flex flex-col gap-3">
                                <div class="flex items-center justify-between">
                                    ...
                                    <span class="font-semibold text-sm">Bank Central Asia (BCA)</span>
                                    <input type="hidden" id="bank" name="bank" value="BCA">
                                </div>
                                <div class="flex items-center justify-between">
                                    <span class="text-sm text-[#66697A]">Nomor Rekening</span>
                                    <span class="font-semibold text-sm">248020770</span>
                                    <input type="hidden" id="number" name="number" value="893092093">
                                </div>
                                <div class="flex items-center justify-between">
                                    <span class="text-sm text-[#66697A]">Nama Akun</span>
                                    <span class="font-semibold text-sm">Muhammad Rikza As-subhy</span>
                                    <input type="hidden" id="account" name="account" value="Muhammad Rikza As-subhy">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex flex-col gap-[10px]">
                    ...
                    <div class="flex items-center w-full p-[14px_16px] rounded-2xl border border-[#E8E9EE] focus-within:border-[#292E4B] transition-all duration-300">
                        ...
                        <input type="text" class="font-semibold placeholder:text-[#292E4B] placeholder:font-normal w-full outline-none" placeholder="Masukkan nama kamu" name="name" id="name">
                    </div>
                </div>
                <div class="flex flex-col gap-[10px]">
                    ...
                    <div class="flex items-center w-full p-[14px_16px] rounded-2xl border border-[#E8E9EE] focus-within:border-[#292E4B] transition-all duration-300">
                        ...
                        <input type="number" class="font-semibold placeholder:text-[#292E4B] placeholder:font-normal w-full outline-none" placeholder="Masukkan NO WA kamu" id="phone_number" name="phone_number">
                    </div>
                </div>
                {{-- <div class="flex flex-col gap-[10px]">
                    ...
                    <div class="relative">
                        <button type="button" class="p-[14px_16px] rounded-2xl flex gap-[10px] w-full border border-[#E8E9EE] focus-within:border-[#292E4B] transition-all duration-300" onclick="document.getElementById('file').click()">
                            ...
                            <p id="fileLabel">Lampirkan Gambar</p>
                        </button>
                ...
                <div class="flex flex-col gap-[10px]">
                    ...
                    <div class="flex w-full p-[14px_16px] rounded-2xl border border-[#E8E9EE] focus-within:border-[#292E4B] transition-all duration-300">
                        ...
                        <textarea name="notes" id="notes" class="font-semibold placeholder:text-[#292E4B] placeholder:font-normal w-full outline-none" cols="30" rows="4" placeholder="Tuliskan Pesan Indah Untuk Para Petani"></textarea>
                    </div>
                </div>
                <button type="type" class="p-[14px_20px] bg-[#76AE43] rounded-full text-white w-full mx-auto font-semibold hover:shadow-[0_12px_20px_0_#76AE4380] transition-all duration-300 text-nowrap text-center">Donasi Sekarang</a>
            </form>
        </div>
    </div>
...
<script>
function submitForm() {
    $.post("{{ route('front.submitDonation') }}",
    {
        _method: 'POST',
        _token: '{{ csrf_token() }}',
        ...
        fundraising_id: $('input#fundraising_id').val(),
        fundraising_slug: $('input#fundraising_slug').val(),
    },
                ...
                onSuccess: function (result) {
                    $.post("{{ route('front.updateStatus') }}",
                    {
                        ...
                        if (data.status == 'error') {
                            ...
                            window.location.href = "{{ route('front.checkout_success', ['fundraising' => $fundraising->slug, 'totalAmountDonation' => $totalAmountDonation])}}";
