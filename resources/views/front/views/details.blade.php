...
@section('title', 'Satu langkah lagi')
...
<section class="max-w-[640px] w-full min-h-screen mx-auto flex flex-col bg-white overflow-x-hidden">
    <div class="header flex flex-col bg-[#56BBC5] overflow-hidden h-[350px] relative -mb-[92px]">
        <nav class="pt-5 px-3 flex justify-between items-center relative z-20">
            ...
            <div class="flex flex-col items-center text-center">
                <p class="text-xs leading-[18px] text-white">Detail Proyek</p>
                <p class="font-semibold text-sm text-white">#AyoMakmurkanMereka</p>
            </div>
    ...
    <div class="flex flex-col z-30">
        ...
        <div id="status" class="w-full h-[92px] bg-[#76AE43] rounded-t-[40px] pt-3 pb-[50px] flex gap-2 justify-center items-center -mb-[38px]">
            ...
            <p class="font-semibold text-sm text-white">Proyek ini sudah selesai</p>
        </div>
        @else
        <div id="status" class="w-full h-[92px] bg-[#FF7815] rounded-t-[40px] pt-3 pb-[50px] flex gap-2 justify-center items-center -mb-[38px]">
            ...
            <p class="font-semibold text-sm text-white">Setiap orang berhak mendapatkan kebaikan mu</p>
        </div>
        @endif
        ...
        <div id="content" class="w-full bg-white rounded-t-[40px] flex flex-col gap-5 p-[30px_24px_120px]">
            <div class="flex flex-col gap-[10px]">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div class="flex gap-1 items-center">
                        <p class="font-semibold text-sm">                        <p class="font-semibold sm:font-medium text-xs leading-[18px]">{{$fundraising->fundraiser->user->name}}</p>
            ...
            <div class="flex flex-col gap-2">
                ...
                <div class="flex items-center justify-between">
                    <p class="text-sm text-[#66697A]">Rp {{number_format($fundraising->totalReachAmount(),0,',','.')}}</p>
            ...
            <div class="flex flex-col gap-[2px]">
                <h2 class="font-semibold text-sm">Deskripsi Proyek</h2>
                <p class="desc-less text-sm leading-[26px]">{{$fundraising->about}} 
                    {{-- <button class="text-[#FF7815] underline" onclick="toggleSeeMoreLess()">besarkan</button> --}}
                </p>
                {{-- <p class="desc-more text-sm leading-[26px] hidden">Kebakaran Hutan dan Lahan (Karhutla) yang terjadi di sejumlah titik di wilayah Kalimantan Barat (Kalbar) selama beberapa Lorem ipsum dolor sit amet, consectetur adipisicing elit. Facilis a earum iure nihil voluptas tenetur. <button class="text-[#FF7815] underline" onclick="toggleSeeMoreLess()">kecilkan</button></p> --}}
            </div>
            @forelse ($fundraising->fundraisingPhases as $phase)
            ...
            <div class="flex flex-col gap-3">
                <div class="flex items-center justify-between">
                    <h2 class="font-semibold text-sm">Teman Donatur ({{$fundraising->donaturs->count()}})</h2>
                   ...
                   <div class="flex items-center gap-3">
                    ...
                    <div class="flex flex-col gap-[2px] w-full">
                        <div class="flex items-center justify-between">
                            ...
                            <p class="font-semibold text-[10px] leading-[15px] text-right text-[#66697A]">dari {{$donatur->name}}</p>
                        </div>
                        <p class="caption text-xs leading-[18px] text-[#66697A]">{{$donatur->notes}}</p>
                    </div>
                </div>
                   @empty
                       <p>Maaf, Belum ada teman donatur dalam proyek ini</p>
                   @endforelse
    ...
    <a href="{{route('front.support', $fundraising->slug)}}" class="p-[14px_20px] bg-[#76AE43] rounded-full text-white w-fit mx-auto font-semibold hover:shadow-[0_12px_20px_0_#76AE4380] transition-all duration-300 fixed bottom-[30px] transform -translate-x-1/2 left-1/2 z-40 text-nowrap">Donasi Sekarang</a>
    @endif
</section>
@endsection