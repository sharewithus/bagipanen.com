            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
               ...
               <div class="item-card flex flex-row justify-between items-center">
                ...
                <div>
                    ...
                    <h3 class="text-indigo-950 text-xl font-bold">Rp {{number_format($wd->amount_requested, '0',',','.')}}</h3>
                </div>
                <div>
                    <p class="text-slate-500 text-sm">Tanggal</p>
                    <h3 class="text-indigo-950 text-xl font-bold">{{$wd->created_at}}</h3>
                </div>
                <div class="hidden md:flex flex-col">
                    <p class="text-slate-500 text-sm">Pemilik Proyek</p>
                    <h3 class="text-indigo-950 text-xl font-bold">{{$wd->fundraiser->user->name}}</h3>
                </div>
                @if ($wd->has_sent)
                        @if ($wd->has_received)
                ...
                <div class="hidden md:flex flex-row items-center gap-x-3">
                    <a href="{{route('admin.fundraising_withdrawals.show', $wd)}}" class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full">
                        Lihat Detail
                    </a>
                </div>
            </div>
