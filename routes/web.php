...
Route::get('/checkout/success/{fundraising:slug}/{totalAmountDonation}', [FrontController::class, 'checkoutSuccess'])->name('front.checkout_success');

Route::post('/checkout/submit', [FrontController::class, 'submitDonation'])->name('front.submitDonation');
Route::post('/checkout/update', [FrontController::class, 'updateStatus'])->name('front.updateStatus');
...
Route::middleware('auth')->group(function () {
    ...
    Route::prefix('admin')->name('admin.')->group(function () {
        ...
        Route::post('/fundraising/active/{fundraising}', [FundraisingController::class, 'activate_fundraising'])->middleware('role:owner')->name('fundraising_withdrawals.activate_fundraising');

        Route::post('/fundraiser/apply', [DashboardController::class, 'apply_fundraiser'])->name('fundraiser.apply');

        Route::get('/my-withdrawals', [DashboardController::class, 'my_withdrawals'])->name('my_withdrawals');

        Route::get('/my-withdrawals/details/{fundraisingWithdrawals}', [DashboardController::class, 'my_withdrawals_details'])->name('my_withdrawals.details');
    });
